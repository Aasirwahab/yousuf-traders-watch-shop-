...
function WatchesContent({
  ...
  return (
    <>
      <section className="border-b border-[#cbd2d2] px-5 pb-6 pt-8 md:px-[4.5%] md:pb-10 md:pt-12">
        <div className="mx-auto max-w-[1440px]">
          ...
          <div className="mt-8 grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
              ...
              <p className="mt-6 max-w-sm text-[13px] leading-6 text-[#687276]">
                Independent and iconic timepieces, curated with a collector&apos;s eye.
              </p>
            </div>
            <div className="hidden items-center gap-3 rounded-full border border-[#cbd2d2] px-4 py-3 text-[11px] uppercase tracking-[0.12em] text-[#687276] md:flex">
              <span className="h-2 w-2 rounded-full bg-[#16343d]" />
              ...
              <button
                ...
                className={`h-10 shrink-0 rounded-full border px-5 text-[11px] font-semibold uppercase tracking-[0.08em] transition-colors ${
                  activeCategory === category
                    ? "border-[#16343d] bg-[#16343d] text-[#eef0ef]"
                    : "border-[#cbd2d2] bg-[#eef0ef] text-[#101416] hover:border-[#0f252b]"
                }`}
              >
                {category}
              </button>
      ...
      <section className="px-5 py-6 md:px-[4.5%] md:py-8">
        <div className="mx-auto max-w-[1440px]">
          <div className="sticky top-0 z-30 -mx-5 border-b border-[#cbd2d2] bg-[#fbfcfb]/95 px-5 py-4 backdrop-blur-xl md:top-0 md:mx-0 md:rounded-none md:px-0">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <button
                type="button"
                onClick={() => setDrawerOpen(true)}
                className="inline-flex h-11 items-center gap-2 rounded-full border border-[#cbd2d2] px-4 text-[12px] font-medium lg:hidden"
              >
                <Filter className="h-4 w-4 stroke-[1.4]" />
                Filters
                {activeFilterCount ? <span className="grid h-5 min-w-5 place-items-center rounded-full bg-[#16343d] px-1 text-[10px] text-[#eef0ef]">{activeFilterCount}</span> : null}
              </button>

              <div className="hidden items-center gap-2 text-[12px] text-[#687276] lg:flex">
              ...
              <div className="flex flex-1 items-center justify-end gap-2 sm:flex-none">
                <label className="relative min-w-[132px]">
                  ...
                  <select value={sort} onChange={(event) => setSort(event.target.value)} className="h-11 w-full appearance-none rounded-full border border-[#cbd2d2] bg-[#eef0ef] px-4 pr-9 text-[12px] outline-none focus:border-[#16343d]">
                ...
                <div className="hidden rounded-full border border-[#cbd2d2] p-1 sm:flex">
          ...
          <div className="grid gap-8 pt-6 lg:grid-cols-[260px_1fr] xl:grid-cols-[300px_1fr]">
            ...
            <div>
              <div className="mb-5 flex items-center justify-between text-[11px] uppercase tracking-[0.12em] text-[#687276]">
                <span>{products.length} timepieces</span>
                <span className="hidden sm:inline">Curated independently</span>
              </div>

              {products.length === 0 ? (
                <div className="grid place-items-center rounded-[16px] border border-[#cbd2d2] px-5 py-20 text-center">
                  <p className="text-lg">No watches match your filters.</p>
                  <p className="mt-2 max-w-sm text-sm text-[#687276]">Try removing a filter or two, or reset to see the full collection.</p>
                  <button type="button" onClick={resetFilters} className="mt-6 grid h-11 place-items-center rounded-full bg-[#16343d] px-6 text-sm text-[#eef0ef]">Reset filters</button>
                </div>
              ) : (
              ...
              <div className="mt-12 grid gap-4 rounded-[16px] border border-[#cbd2d2] bg-[#eef0ef] p-5 sm:grid-cols-2 lg:grid-cols-4">
                {trustItems.map((item) => (
                  <div key={item.label} className="flex items-center gap-3 border-[#cbd2d2] text-[11px] font-semibold uppercase tracking-[0.1em] text-[#46555a] lg:border-r last:lg:border-r-0">
              ...
              <div className="mt-8 grid gap-4 rounded-[16px] border border-[#cbd2d2] p-5 md:grid-cols-[1.3fr_1fr] md:items-center">
                <div>
                  <h2 className="text-2xl font-normal tracking-[-0.04em]">Looking for something particular?</h2>
                  <p className="mt-2 max-w-xl text-sm leading-6 text-[#687276]">Our concierge can source a specific reference, arrange a private viewing, or help compare men&apos;s, ladies&apos;, and unisex pieces.</p>
                </div>
                <div className="flex flex-col gap-3 sm:flex-row md:justify-end">
                  <a href="/contact" className="grid h-12 place-items-center rounded-full bg-[#16343d] px-5 text-sm text-[#eef0ef]">Contact concierge</a>
                  <a href="tel:+18336222536" className="grid h-12 place-items-center rounded-full border border-[#16343d] px-5 text-sm">Call boutique</a>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div>
      <div className="mb-5 flex items-center justify-between">
        ...
        <button type="button" onClick={onReset} className="text-[11px] uppercase tracking-[0.12em] text-[#687276] hover:text-[#101416]">Reset</button>
      </div>
          ...
          <details key={key} open={key === "collection" || key === "gender"} className="group py-4">
            ...
            <div className="mt-4 space-y-3">
              {SHOP_FILTERS[key].map((value) => {
                ...
                return (
                  <label key={value} className="flex cursor-pointer items-center justify-between gap-3 text-[13px] text-[#46555a]">
                    <span>{value}</span>
                    <input type="checkbox" checked={active} onChange={() => onToggle(key, value)} className="peer sr-only" />
                    <span className={`grid h-4 w-4 place-items-center border transition-colors ${active ? "border-[#16343d] bg-[#16343d]" : "border-[#cbd2d2] bg-[#eef0ef]"}`}>
                      {active ? <Check className="h-3 w-3 text-[#eef0ef]" /> : null}
                    </span>
                  </label>
                );
              })}
...
function FilterDrawer({ open, onClose, children }: { open: boolean; onClose: () => void; children: React.ReactNode }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-[80] bg-[#0f252b]/45 backdrop-blur-sm lg:hidden" onMouseDown={onClose}>
      <aside role="dialog" aria-modal="true" aria-label="Filter watches" onMouseDown={(event) => event.stopPropagation()} className="ml-auto flex h-full w-[90%] max-w-sm flex-col bg-[#eef0ef]">
        <div className="flex h-16 items-center justify-between border-b border-[#cbd2d2] px-5">
          <h2 className="text-sm font-medium">Refine watches</h2>
          <button type="button" aria-label="Close filters" onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full hover:bg-[#0f252b]/5">
            <X className="h-5 w-5 stroke-[1.3]" />
          </button>
        </div>
        <div className="min-h-0 flex-1 overflow-y-auto p-5">{children}</div>
        <button type="button" onClick={onClose} className="m-5 grid h-12 place-items-center rounded-full bg-[#16343d] text-sm text-[#eef0ef]">View watches</button>
      </aside>
    </div>
  );
}
      ...
      <div className={`relative overflow-hidden rounded-[16px] bg-[#fbfcfb] ${list ? "aspect-square" : "aspect-[0.86]"}`}>
        ...
        {!list ? (
          <button type="button" aria-label={`${favorite ? "Remove" : "Add"} ${product.name} ${favorite ? "from" : "to"} wishlist`} aria-pressed={favorite} onClick={onFavorite} className="absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full bg-[#fbfcfb]/90 backdrop-blur-sm">
            <Heart className={`h-[17px] w-[17px] stroke-[1.4] ${favorite ? "fill-[#16343d] text-[#16343d]" : ""}`} />
          </button>
        ) : null}
      </div>
      <div className={list ? "min-w-0 self-center" : "mt-4"}>
        <div className={list ? "flex items-start justify-between gap-3" : "flex items-start justify-between gap-3"}>
          <div className="min-w-0">
            ...
            <h3 className={`${list ? "mt-1 text-base" : "mt-1 truncate text-[14px]"}`}><Link href={`/watches/${product.slug}`} className="hover:text-[#16343d]">{product.name}</Link></h3>
            <p className="mt-1 text-[10px] text-[#7e8c93]">Ref. {product.reference}</p>
          </div>
          <button type="button" aria-label={`${favorite ? "Remove" : "Add"} ${product.name} ${favorite ? "from" : "to"} wishlist`} aria-pressed={favorite} onClick={onFavorite} className={`${list ? "grid" : "hidden"} h-10 w-10 shrink-0 place-items-center rounded-full border border-[#cbd2d2]`}>
            <Heart className={`h-[17px] w-[17px] stroke-[1.4] ${favorite ? "fill-[#16343d] text-[#16343d]" : ""}`} />
          </button>
        </div>
        <div className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-[#687276]">
        ...
        <div className="mt-3 flex items-center justify-between gap-3">
          ...
          {!list ? (
            <button type="button" onClick={() => commerce.addItem(product.slug)} className="group/button inline-flex h-9 items-center gap-2 rounded-full border border-[#16343d] px-3 text-[11px] transition-colors hover:bg-[#0f252b] hover:text-[#eef0ef]">
              Reserve <ArrowRight className="h-3.5 w-3.5 transition-transform group-hover/button:translate-x-0.5" />
            </button>
          ) : null}
        </div>
      </div>
      {list ? <button type="button" onClick={() => commerce.addItem(product.slug)} className="self-center rounded-full bg-[#16343d] px-5 py-3 text-[12px] text-[#eef0ef] max-sm:col-span-2 max-sm:w-full">Reserve</button> : null}
    </article>
  );
}
...
function IconToggle({ label, active, onClick, children }: { label: string; active: boolean; onClick: () => void; children: React.ReactElement<{ className?: string }> }) {
  return (
    <button type="button" aria-label={label} aria-pressed={active} onClick={onClick} className={`grid h-9 w-9 place-items-center rounded-full transition-colors ${active ? "bg-[#16343d] text-[#eef0ef]" : "text-[#687276] hover:text-[#101416]"}`}>
      {children}
    </button>
  );
}
