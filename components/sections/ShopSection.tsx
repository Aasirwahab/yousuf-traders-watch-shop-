    ...
    <section id="shop" className="border-b border-[#cbd2d2] px-6 py-20 md:px-[4.5%] md:py-28">
      <div className="mx-auto max-w-[1440px]">
        <div className="flex items-end justify-between gap-6">
          ...
          <Link href="#categories" className="hidden items-center gap-3 border-b border-[#16343d] pb-1 text-sm md:flex">View all watches <ArrowRight className="h-4 w-4" /></Link>
        </div>
            ...
            return (
              <article key={product.slug} className="group min-w-0">
                <div className="relative aspect-[0.86] overflow-hidden rounded-[16px] bg-[#fbfcfb]">
                  ...
                  <span className="absolute bottom-3 left-3 bg-[#fbfcfb]/90 px-2.5 py-1 text-[10px] uppercase tracking-[0.12em] backdrop-blur-sm">{product.condition}</span>
                </div>
                <div className="mt-4 flex items-start justify-between gap-3">
                  <div className="min-w-0"><p className="text-[11px] font-semibold uppercase tracking-[0.1em]">{product.brand}</p><h3 className="mt-1 truncate text-[14px]">{product.name}</h3><p className="mt-1 text-[10px] text-[#7e8c93]">Ref. {product.reference}</p><p className="mt-3 text-sm font-medium">{product.price}</p></div>
