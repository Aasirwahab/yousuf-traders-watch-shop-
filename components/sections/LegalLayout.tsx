  ...
  return (
    <main className="min-h-screen bg-[#eef0ef] text-[#101416]">
      ...
      <section className="mx-auto max-w-3xl px-5 pb-24 pt-8 md:px-8 md:pt-12">
        ...
        <p className="mt-4 text-[12px] text-[#7e8c93]">Last updated {updated}</p>
        {intro ? <p className="mt-6 max-w-xl text-sm leading-7 text-[#46555a]">{intro}</p> : null}

        <div className="mt-10 space-y-9">{children}</div>

        {legalNote ? (
          <p className="mt-14 rounded-[12px] border border-[#cbd2d2] bg-[#eef0ef] p-5 text-[12px] leading-6 text-[#687276]">
...
export function LegalSection({ heading, children }: { heading: string; children: ReactNode }) {
  return (
    <section>
      <h2 className="text-[15px] font-semibold tracking-[-0.01em] text-[#101416]">{heading}</h2>
      <div className="mt-3 space-y-3 text-sm leading-7 text-[#46555a]">{children}</div>
    </section>
  );
}
