...
export default function TrustStrip() {
  return (
    <section id="trust" aria-label="Purchase guarantees" className="border-b border-[#cbd2d2] bg-[#eef0ef] px-6 md:px-0">
      <div className="grid grid-cols-2 py-5 md:h-[124px] md:grid-cols-4 md:py-0">
        {ITEMS.map(({ icon: Icon, title }, index) => (
          <div key={title} className={`flex items-center gap-4 py-4 md:justify-center md:gap-6 md:px-5 ${index % 2 ? "border-l border-[#cbd2d2] pl-5" : "pr-5"} ${index > 1 ? "border-t border-[#cbd2d2] md:border-t-0" : ""} ${index > 0 ? "md:border-l md:border-[#cbd2d2]" : ""}`}>
