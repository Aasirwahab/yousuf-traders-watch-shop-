      ...
      <section className="mx-auto max-w-[1440px] px-5 pb-24 pt-8 md:px-[4.5%] md:pt-12">
        ...
        {!mounted ? (
          ...
          <div className="mt-12 grid place-items-center rounded-[16px] border border-[#cbd2d2] px-5 py-20 text-center">
            ...
            <p className="mt-2 max-w-sm text-sm text-[#687276]">Tap the heart on any watch to save it here for later.</p>
            <Link href="/watches" className="mt-6 grid h-11 place-items-center rounded-full bg-[#16343d] px-6 text-sm text-[#eef0ef]">Browse the collection</Link>
          </div>
        ) : (
          <>
            <p className="mt-8 text-[11px] uppercase tracking-[0.12em] text-[#687276]">
                ...
                <article key={product.slug} className="group min-w-0">
                  <div className="relative aspect-[0.86] overflow-hidden rounded-[16px] bg-[#fbfcfb]">
                    ...
                    <button
                      ...
                      className="absolute right-3 top-3 grid h-10 w-10 place-items-center rounded-full bg-[#fbfcfb]/90 backdrop-blur-sm"
                    >
                      <X className="h-[17px] w-[17px] stroke-[1.4]" />
                    </button>
                  </div>
                  <p className="mt-4 text-[10px] font-semibold uppercase tracking-[0.11em]">{product.brand}</p>
                  <h2 className="mt-1 truncate text-[14px]"><Link href={`/watches/${product.slug}`} className="hover:text-[#16343d]">{product.name}</Link></h2>
